...
- **Leadership page** (`leadership.html`): the overview for you and the directors.
  ...
  - The crew list, with PIN resets and leader promotion.
...
Every page runs in demo mode until `config.js` has an API URL. In demo mode, all features work, but data stays in that browser. In demo mode the master PIN is `1234`, the sample crew's PINs are all `1234`, and Alex Rivera is the sample leader.
...
The crew page's header has a red **Emergency** button. It needs no sign-in and opens instantly.

- **Call 911** is the big button at the top, since a serious injury shouldn't wait on a phone tree.
- **The shop address** is shown next, so whoever calls can say where to send the ambulance.
- **Four contacts** follow: both co-directors, the set designer, and the theater director. Each row dials with one tap.
- **While you wait:** stop and unplug the saws, don't move someone who fell or hit their head, and where the first aid kit is.

Fill in `emergencyContacts` and `shopAddress` in `config.js`. The button stays hidden until at least one number has been entered. Numbers can be written however you like, including `(979) 555-0134` or `979.555.0155 x2`; extensions are dialed after a pause.

### Leadership access

The crew page's header has a **Leadership login** button, but the leadership page is closed until someone signs in. The data is withheld by the sheet itself, not just hidden on screen, so the link alone shows nothing.

**Two ways in:**
- **A leader's own name and 4-digit PIN,** the same ones they use on the crew page. Only people flagged as leaders get through.
- **The master PIN** from Apps Script (Project Settings > Script properties > LEAD_PIN). Use it to get in the first time, and to rescue access if leaders are locked out.
